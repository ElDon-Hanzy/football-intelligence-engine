# C0204 — Continuous Projection Coverage Reconciliation V02
...
Migration: `20260905231505_c0204_continuous_projection_coverage_reconciliation_v02.sql`
...
- finds only current `UNGOVERNED_MISSING` rows from the canonical C0204 coverage function;
...
- is idempotent while the governed exclusion remains the latest eligibility event.

The production wrapper now performs:

`coverage check -> govern new missing players -> coverage recheck -> block only if ungoverned missing remains -> existing C0160 generator`
...
Samba's latest eligibility state is now:
...
Security and performance advisors were run after the migration. They reported existing project-level advisory items, but no new warning was attributable to the C0204 V02 function or wrapper. The new SECURITY DEFINER function is in the private schema, has an explicit fixed search path, and execute privilege is restricted to `service_role`.
