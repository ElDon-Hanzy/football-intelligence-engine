...
declare
  ...
  i integer; g integer; v_fixtures integer; v_prod integer; v_tactical integer; v_features integer; v_prod_at timestamptz; v_deadline timestamptz;
  v_run_id bigint; v_run_at timestamptz; v_run_rows integer; v_dist_rows integer; v_cov_projectable integer; v_cov_ungoverned integer;
  v_upstream boolean; v_projection boolean; v_snapshot_complete boolean; v_cadence_valid boolean; v_upstream_drifted boolean; v_cadence_mode text; v_weights text;
  v_optimizer_contract text:='C0218_FULL_POOL_SCENARIO_V03';
begin
  ...
  for i in 0..p_horizon-1 loop
    ...
    from public.matches
    where source='fpl' and gameweek=g;
    ...
    select id,generated_at into v_run_id,v_run_at
    from public.gameweek_prediction_runs
    where gameweek=g and model_version_id=v_active_model_id
      and run_type='pre_deadline' and frozen=true
      and (v_deadline is null or generated_at<v_deadline)
    ...
    if v_run_id is not null then
      ...
      from public.fpl_projection_coverage_audits
      where gameweek=g and prediction_run_id=v_run_id
      order by captured_at desc,id desc
      limit 1;
    else
    ...
    v_snapshot_complete:=v_run_id is not null and v_run_rows>=500 and v_dist_rows=v_run_rows
      and coalesce(v_cov_projectable,v_run_rows)=v_run_rows and coalesce(v_cov_ungoverned,0)=0;

    if i=0 then
      if v_deadline is not null and v_now>=v_deadline-interval '2 hours' then
        v_cadence_mode:='PRIMARY_FINAL_T_MINUS_2H';
        v_cadence_valid:=v_run_at is not null and v_run_at>=v_deadline-interval '2 hours' and v_run_at<v_deadline;
      else
        v_cadence_mode:='PRIMARY_DAILY_24H';
        v_cadence_valid:=v_run_at is not null and v_run_at>v_now-interval '24 hours';
      end if;
    else
      v_cadence_mode:='FORWARD_BASELINE';
      v_cadence_valid:=v_run_at is not null;
    end if;

    v_upstream_drifted:=v_run_at is not null and v_run_at<greatest(coalesce(v_prod_at,'epoch'::timestamptz),coalesce(v_player_state_at,'epoch'::timestamptz));
    v_projection:=v_upstream and v_snapshot_complete and v_cadence_valid;
    ...
    v_items:=v_items||jsonb_build_array(jsonb_build_object(
      ...
      'required_fixture_generator','production_fixture_v0.3_c0166','tactical_rows',v_tactical,'feature_rows',v_features,
      'upstream_ready',v_upstream,'prediction_run_id',v_run_id,'prediction_run_at',v_run_at,'projection_rows',v_run_rows,
      'distribution_rows',v_dist_rows,'projectable_rows',v_cov_projectable,'ungoverned_missing',v_cov_ungoverned,
      'snapshot_complete',v_snapshot_complete,'cadence_mode',v_cadence_mode,'cadence_valid',v_cadence_valid,
      'upstream_drifted_since_snapshot',v_upstream_drifted,'projection_ready',v_projection
    ));
  end loop;
  ...
  if v_all_projection and v_manager_ready then
    v_sig:=md5(concat_ws('|','C0218_OPT_V03',v_optimizer_contract,v_gw,p_horizon,v_active_model_id,v_manager_state_id,v_ft,v_budget,4,0.12,1.0,v_weights,
      (select string_agg(coalesce(x->>'run_id','null'),',' order by (x->>'gameweek')::integer) from jsonb_array_elements(v_runs) x)));
  end if;

  return jsonb_build_object(
    'ok',true,'change_id','C0218','reconciliation_change_id','C0219','contract_version','C0218_HORIZON_V03',
    'cadence_contract_version','C0217_PROJECTION_CADENCE_V01','captured_at',v_now,'gameweek',v_gw,'horizon',p_horizon,
    'active_model_id',v_active_model_id,'player_state_at',v_player_state_at,
    'manager_state',jsonb_build_object('id',v_manager_state_id,'captured_at',v_manager_at,'free_transfers',v_ft,'bank_tenths',v_bank,
      'acquisition_squad_cost_tenths',v_acq,'squad_liquidation_value_tenths',v_liq,'optimizer_budget_tenths',v_budget,'ready',v_manager_ready),
    ...
    'optimizer_policy',jsonb_build_object('budget_tenths',v_budget,'transfer_cost_points',4,'bench_weight',0.12,'model_error_margin_points',1.0,'weights',string_to_array(v_weights,',')::numeric[]),
    'projection_readiness_policy','CADENCE_VALID_SNAPSHOT_NOT_LIVE_UPSTREAM_TIMESTAMP_PARITY','missing_data_is_not_zero',true,'historical_forecasts_rewritten',false
  );
end $function$;
...
as $function$
declare
  v_capture jsonb; v_cadence jsonb; v_ready jsonb; v_status jsonb; v_gw integer; v_sig text; v_req_id bigint; v_body jsonb; v_weights jsonb; v_manager jsonb;
  v_last_req_at timestamptz; v_last_http integer; v_budget integer;
begin
  ...
  v_cadence:=private.c0217_projection_horizon_cycle_v01();
  v_ready:=private.c0213_p2_horizon_readiness_v02(p_gameweek,p_horizon);
  if not coalesce((v_ready->>'ok')::boolean,false) then return v_ready||jsonb_build_object('capture',v_capture,'cadence',v_cadence); end if;
  v_gw:=(v_ready->>'gameweek')::integer;

  if not coalesce((v_ready->>'upstream_horizon_ready')::boolean,false) then
    return jsonb_build_object('ok',true,'status','WAITING_FOR_HORIZON_UPSTREAM','gameweek',v_gw,'horizon',p_horizon,'capture',v_capture,'cadence',v_cadence,'readiness',v_ready,'decisioning',false,'writes_manager_plan',false);
  end if;
  if not coalesce((v_ready->>'projection_horizon_ready')::boolean,false) then
    return jsonb_build_object('ok',true,'status','WAITING_FOR_PROJECTION_CADENCE','gameweek',v_gw,'horizon',p_horizon,'capture',v_capture,'cadence',v_cadence,'readiness',v_ready,'decisioning',false,'writes_manager_plan',false,'reconciliation_policy','C0217_CADENCE_CONTROLLER_ONLY_NO_DIRECT_GENERATOR_CALL');
  end if;
  if not coalesce((v_ready->>'ready_for_optimizer')::boolean,false) then
    return jsonb_build_object('ok',true,'status','WAITING_FOR_MANAGER_STATE','gameweek',v_gw,'horizon',p_horizon,'capture',v_capture,'cadence',v_cadence,'readiness',v_ready,'decisioning',false,'writes_manager_plan',false);
  end if;

  v_status:=private.c0213_p2_optimizer_status_v01(v_gw,p_horizon);
  if coalesce((v_status->>'ready')::boolean,false) then return v_status||jsonb_build_object('capture',v_capture,'cadence',v_cadence,'decisioning',false,'reconciliation_change_id','C0219'); end if;
  ...
  left join net._http_response resp on resp.id=q.request_id
  where q.input_signature=v_sig
  ...
  if v_last_req_at is not null and v_last_req_at>clock_timestamp()-interval '10 minutes' and (v_last_http is null or v_last_http=200) then
    return v_status||jsonb_build_object('capture',v_capture,'cadence',v_cadence,'decisioning',false,'reconciliation_change_id','C0219');
  end if;
  ...
  insert into private.c0213_p2_optimizer_requests(request_id,gameweek,horizon,input_signature,manager_state_id,free_transfers,budget_tenths,prediction_runs,request_body,change_id)
  values(v_req_id,v_gw,p_horizon,v_sig,(v_manager->>'id')::bigint,(v_manager->>'free_transfers')::integer,v_budget,v_ready->'prediction_runs',v_body,'C0219');
  return jsonb_build_object('ok',true,'status','DISPATCHED','gameweek',v_gw,'horizon',p_horizon,'request_id',v_req_id,'input_signature',v_sig,'capture',v_capture,'cadence',v_cadence,'readiness',v_ready,'decisioning',false,'writes_manager_plan',false,'reconciliation_change_id','C0219');
end $function$;
