...
function completeFrozenDecision(row: any): boolean {
  return Boolean(
    row &&
    FROZEN_SOURCES.includes(String(row.source || '')) &&
    Array.isArray(row.starting_xi) && row.starting_xi.length === 11 &&
    Array.isArray(row.bench_order) && row.bench_order.length === 4 &&
    Number.isFinite(Number(row.captain_player_id)) &&
    Number.isFinite(Number(row.vice_player_id)),
  );
}
...
function frozenResponse(row: any, gameweek: number, entryId: number, status = 'FROZEN_DB_SNAPSHOT') {
  return Response.json({
    ...
    vice_player_id: Number(row.vice_player_id),
    chip: row.chip ?? null,
    semantics: {
      actual_snapshot_is_immutable_after_first_complete_locked_capture: true,
      no_fpl_request_was_made: true,
      frontend_reads_database_only: true,
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // C0258 immutable gate: after one complete locked FPL snapshot exists, stop here.
    // This path does not contact fantasy.premierleague.com at all.
    if (gameweek) {
      const existing = await latestActual(sb, gameweek);
      if (completeFrozenDecision(existing)) return frozenResponse(existing, gameweek, entryId);
    }
    ...
    const now = Date.now();
    const targetEvent = gameweek
      ? events.find((event: any) => Number(event.id) === gameweek)
      : [...events].filter((event: any) => new Date(event.deadline_time).getTime() <= now).sort((a: any, b: any) => Number(b.id) - Number(a.id))[0];
    ...
    if (!deadlineAt || now < new Date(deadlineAt).getTime()) {
      return Response.json({ ok: true, change_id: 'C0258', gameweek, status: 'PRE_DEADLINE_HIDDEN', persisted: false, deadline_at: deadlineAt || null, semantics: { predeadline_hidden_picks_are_not_inferred: true } });
    }

    // Re-check after the external bootstrap in case another invocation froze the GW concurrently.
    ...
    return Response.json({ ok: false, change_id: 'C0258', error: error instanceof Error ? error.message : String(error) }, { status: 500 });
  }
});
