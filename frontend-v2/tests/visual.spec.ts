...
const fixtureResults = Array.from({ length: 10 }, (_, index) => {
  ...
  return {
    ...
    home_short: index === 0 ? 'IPS' : index === 1 ? 'FUL' : `H${index}`,
    away_short: index === 0 ? 'LIV' : index === 1 ? 'CRY' : `A${index}`,
    finished: false,
    prediction: {
...
const factsPayload = {
  ...
  fixtures: fixtureResults.map((fixture, index) => {
    const mkFact = (id: number, alignment: 'SUPPORTS' | 'CONTRADICTS', text: string, rank: number) => ({
      id, snapshot_run_id: 7, match_id: fixture.match_id, team_id: 10 + index * 2, opponent_team_id: 11 + index * 2,
      fact_type: alignment === 'SUPPORTS' ? 'C0166_MATCHUP_XG' : 'C0166_CURRENT_SEASON_PROCESS', usefulness_score: .9 - rank * .05,
      card_rank: rank, alignment, one_liner: text, payload: { family: alignment }, evidence_cutoff: '2026-09-01T20:32:50+00:00',
    });
    return {
      ...
      card_facts: [mkFact(1000 + index, 'SUPPORTS', `${fixture.home_team} have the stronger recent chance matchup.`, 1)],
      modal_facts: [mkFact(2000 + index, 'SUPPORTS', `${fixture.home_team} have the stronger recent chance matchup.`, 1), mkFact(3000 + index, 'CONTRADICTS', `${fixture.away_team} retain a credible counter-signal.`, 2)],
    };
  }),
};

const bettingPayload = {
  ok: true, gameweek: 3, odds_status: 'connected', value_edge_available: false, research_edge_available: true,
  price_tracking_available: false, clv_research_available: false, warnings: [],
  fixtures: fixtureResults.map((fixture) => ({
    match_id: fixture.match_id, kickoff_time: fixture.kickoff_time, finished: false, home_team: fixture.home_team, away_team: fixture.away_team,
    ...
    bookmaker_odds: [
      { bookmaker: 'Book A', bookmaker_family: 'Book A', market_key: 'h2h', selection_key: 'home', selection_name: fixture.home_team, decimal_odds: 2.4, implied_probability: .4167 },
      { bookmaker: 'Book A', bookmaker_family: 'Book A', market_key: 'h2h', selection_key: 'draw', selection_name: 'Draw', decimal_odds: 3.4, implied_probability: .2941 },
      { bookmaker: 'Book A', bookmaker_family: 'Book A', market_key: 'h2h', selection_key: 'away', selection_name: fixture.away_team, decimal_odds: 3.0, implied_probability: .3333 },
    ],
    correct_score_odds: [{ bookmaker: 'Book A', bookmaker_family: 'Book A', market_key: 'correct_score', selection_key: '1-1', selection_name: '1-1', decimal_odds: 7.5, implied_probability: .1333 }],
    bookmaker_count: 1, bookmaker_source_count: 1, market_count: 2,
    edge_research: { status: 'UNVALIDATED', model_effect_enabled: false, observation_count: 3, robust_positive_ev_count: 1, top_robust_positive_ev: [] },
    price_tracking: null, clv_research: null,
  })),
};
...
const calibrationPayload = {
  ...
  validation: { forward: { available: true, selected_ablation_key: 'A0005', coverage: { predictions: 140, evaluations: 70, cohort_fixtures: 20 }, integrity: { prediction_actual_data_violations: 0 }, variants: [
  ...
  ], latest_promotion_gate: null }, retrospective: [{ gameweek: 2, evaluated_fixtures: 10, direction_accuracy: .6, exact_top_score_rate: .2, avg_brier: .63, avg_score_log_loss: 3.13 }] },
};

const enginePayload = {
  ok: true, gameweek: 3, generated_at: '2026-09-02T00:55:00Z', active_model: { id: 3, version: '0.3', description: 'Current model', config: {}, is_active: true, created_at: '2026-08-20T00:00:00Z' },
...
const routes = [
  ['home', 'Command Center'], ['fixtures', 'Fixtures'], ['fpl', 'FPL decision workspace'], ['markets', 'Markets'], ['performance', 'Performance'], ['engine', 'Engine & Research'],
] as const;
...
test('C0175 deterministic visual baselines cover every v2 surface', async ({ page }, testInfo) => {
  ...
  for (const [view, heading] of routes) {
    ...
    console.log(`C0175_VISUAL|${key}|${hash}`);
