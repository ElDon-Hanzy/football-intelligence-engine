...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [activeRes, runRes, fixtureRes, cacheRes] = await Promise.all([
      ...
      sb.rpc('engine_diagnostics_cached_status_v01', { p_gameweek: gameweek }),
    ]);
    for (const result of [activeRes, runRes, fixtureRes, cacheRes]) if (result.error) throw result.error;
    ...
    const cacheEnvelope: any = cacheRes.data || null;
    const status: any = cacheEnvelope?.status || {};
    if (!cacheEnvelope?.status) throw new Error(`Diagnostics cache unavailable for GW${gameweek}`);

    return new Response(JSON.stringify({
      ok: true,
      gameweek,
      generated_at: cacheEnvelope.captured_at || new Date().toISOString(),
      ...
      cache: { captured_at: cacheEnvelope.captured_at || null, source: 'C0239_ENGINE_DIAGNOSTICS_CACHE_V01' },
      semantics: {
        ...
        diagnostics_served_from_cached_audit_state: true,
      },
    }), { headers: cors });
  } catch (error) {
