...
test('matchup modal keeps frozen probabilities and compact result comparison', async ({ page }) => {
  await mockProductApis(page); await page.goto('/?gw=4#matches'); const fixture = page.locator('.v3-match-card').first(); await expect(fixture).toBeVisible(); await fixture.getByRole('button', { name: 'Details' }).click(); const dialog = page.getByRole('dialog'); await expect(dialog).toBeVisible(); await expect(dialog.getByText('Probability board', { exact: true })).toBeVisible(); await expect(dialog.getByRole('heading', { name: /Counterpoints \/ risks/ })).toBeVisible(); await expect(dialog.getByText('Transition exposure remains a credible counterpoint.', { exact: true })).toBeVisible();
});
