import { fetchFplWorkspace } from './fplWorkspace';
...
export async function fetchGameweekCatalog(signal?: AbortSignal): Promise<GameweekCatalog> {
  try {
    ...
    const workspace = await fetchFplWorkspace(0, signal);
    return { currentGameweek: workspace.gameweek, latestIntelligenceGameweek: workspace.gameweek };
  }
}
