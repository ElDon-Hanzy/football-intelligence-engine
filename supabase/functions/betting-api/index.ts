...
Deno.serve(async req=>{
  ...
  try{
    ...
    const [mr,tr,sr,ir,psr,clvr]=await Promise.all([
      ...
      read('predictions',()=>sb.from('current_fixture_decision_contract_v01').select('snapshot_id,match_id,captured_at,home_lambda,away_lambda,top_scorelines,markets,confidence,source_snapshot,representative_score,representative_score_probability,raw_modal_score,raw_modal_probability,decision_contract_version,result_decision,direction_strength,primary_environment,scoring_environment_probabilities,scoring_environment_state,dominant_subtype,selected_family,selected_family_probability,chronology_and_coverage_valid,decision_hash').eq('gameweek',gw).order('captured_at',{ascending:false}).order('snapshot_id',{ascending:false})),
