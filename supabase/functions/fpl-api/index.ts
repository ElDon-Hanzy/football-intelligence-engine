...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const[{data:mv},{data:activeMv},{data:decRaw},{data:rr},fr,{data:fixturePreds,error:fpe},{data:prefinal}]=await Promise.all([
    ...
    if(fpe)throw fpe;if(!fr.ok)throw new Error(`FPL fixtures ${fr.status}`);
    const fixtures=await fr.json(),liveFixtureById=new Map((fixtures||[]).map((f:any)=>[Number(f.id),f])),finishedLive=new Set((fixtures||[]).filter((f:any)=>f.finished===true||f.finished_provisional===true).map((f:any)=>Number(f.id))),snapshotFinishedIds=Array.isArray(rr?.metadata?.finished_fixture_ids)?new Set((rr.metadata.finished_fixture_ids||[]).map((x:any)=>Number(x))):null,rrObserved=rr?ts(rr.observed_at):0,dec=historicalProjectionValid?decRaw:null;
    let ids:number[]=[];
    ...
    return new Response(JSON.stringify({ok:true,contract_version:'fpl_api_v12_chronology_safe',available_gameweeks,gameweek:gw,prediction_run_id:run.id,model_version:mv?.version||'unknown',current_model_version:activeMv?.version||null,generated_at:run.generated_at,deadline_at:run.deadline_at||(Number.isFinite(derivedDeadline)?new Date(derivedDeadline).toISOString():null),run_type:run.run_type,excluded_from_backtest:Boolean(run.excluded_from_backtest),historical_projection_valid:historicalProjectionValid,historical_unavailable_reason:historicalProjectionValid?null:'NO_VALID_PREDEADLINE_PROJECTION',snapshot_stage:snapshotStage,prefinal:prefinal||null,metadata_availability:metadataAvailability,notes:run.notes,previous_gameweek:prevRun?.gameweek||null,decision:dec?{starting_xi:dec.starting_xi,bench:dec.bench_order,captain_player_id:dec.captain_player_id,vice_player_id:dec.vice_player_id,recommendations:dec.recommendations}:null,result_run:rr||null,audit,finished_fixture_ids:[...finishedLive],fixture_results,squad,top_double_digit,all_predictions}),{headers:cors});
