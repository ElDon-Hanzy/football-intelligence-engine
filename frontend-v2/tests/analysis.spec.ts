...
const fixtures = Array.from({ length: 10 }, (_, index) => ({
  ...
  prediction: {
    ...
    markets: index === 0 ? { home_win: .3819, draw: .2433, away_win: .3745 } : { home_win: .52, draw: .25, away_win: .23 },
    top_scorelines: [{ score: index === 0 ? '1-1' : '2-1', prob: .112 }],
  },
  ...
  edge_research: index === 0 ? { status: 'UNVALIDATED', model_effect_enabled: false, observation_count: 3, robust_positive_ev_count: 1, top_robust_positive_ev: [] } : null,
  price_tracking: null,
  clv_research: null,
}));
...
const bettingPayload = {
  ...
  price_tracking_available: false, clv_research_available: false, warnings: [], fixtures,
};
...
const calibrationPayload = {
  ...
  validation: {
    forward: {
      available: true, selected_ablation_key: 'A0005', coverage: { predictions: 140, evaluations: 70, cohort_fixtures: 20, splits: [{ split: 'VALIDATION', gameweek: 2, fixtures: 10 }, { split: 'TEST', gameweek: 3, fixtures: 10 }] },
    ...
    retrospective: [{ gameweek: 2, evaluated_fixtures: 10, direction_accuracy: .6, exact_top_score_rate: .2, avg_brier: .63, avg_score_log_loss: 3.13 }],
  },
};
...
test('Markets stays decision-safe while exposing model/price context', async ({ page }) => {
  ...
  const research = page.locator('.market-card').first().getByText('Unvalidated research', { exact: true });
