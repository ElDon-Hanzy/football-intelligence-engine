...
Deno.serve(async req=>{
  try{
    ...
    const [mr,tr,sr,ir]=await Promise.all([
    ...
    for(const [name,res] of [['matches',mr],['teams',tr],['predictions',sr],['ingestion_run',ir]] as any){if(res.error)throw Error(`${name}: ${res.error.message}`)}
