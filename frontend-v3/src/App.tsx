...
export function App() {
  ...
  useEffect(() => { const onHashChange = () => { setActive(viewFromLocation()); window.scrollTo({ top: 0, behavior: 'auto' }); }; window.addEventListener('hashchange', onHashChange); return () => window.removeEventListener('hashchange', onHashChange); }, []);
  let content;
  ...
  else if (active === 'markets') content = <MarketsPage />;
  else if (active === 'insights') content = <InsightsPage />;
  else content = <HistoryPage />;
  return <div className="v3-app-shell">
    <header className="v3-topbar"><a className="v3-brand" href="#home" aria-label="Football Intelligence home"><span className="v3-brand-mark" aria-hidden="true">FI</span><span><strong>Football Intelligence</strong><small>V3</small></span></a><nav className="v3-desktop-nav" aria-label="Primary navigation">{navItems.map((item) => <a key={item.view} href={`#${item.view}`} aria-current={item.view === active ? 'page' : undefined}>{item.label}</a>)}</nav><div className="v3-top-actions"><span className="v3-status" data-tone="intelligence">Live product</span></div></header>
    <main className="v3-page" id={active} data-active-view={active}>{content}</main>
