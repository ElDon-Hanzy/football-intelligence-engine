...
const currentFixtures = [
  { match_id: 1, fpl_fixture_id: 1, kickoff_time: '2026-09-12T14:00:00Z', home_team: 'Chelsea', away_team: 'Hull City', home_short: 'CHE', away_short: 'HUL', finished: true, home_score: 2, away_score: 2, prediction: { snapshot_id: 9001, source_change_id: 'C0284', captured_at: '2026-09-12T12:00:00Z', markets: { home_win: .58, draw: .25, away_win: .17 }, result_decision: 'HOME', direction_strength: 'STRONG', primary_environment: 'NORMAL_SCORING', scoring_environment_probabilities: { low: .2, normal: .5, high: .3 }, scoring_environment_state: 'PRIMARY', expected_total_goals: 2.9, representative_score: '2-1', representative_score_probability: .12, raw_modal_score: '1-1', raw_modal_probability: .11, chronology_and_coverage_valid: true, decision_contract_version: 'c0284_fixture_decision_v01', decision_hash: 'fixture-1', top_scorelines: [{ score: '2-1', prob: .12 }, { score: '1-1', prob: .11 }] } },
  { match_id: 2, fpl_fixture_id: 2, kickoff_time: '2026-09-12T19:00:00Z', home_team: 'Sunderland', away_team: 'Arsenal', home_short: 'SUN', away_short: 'ARS', finished: false, home_score: 0, away_score: 1, prediction: { snapshot_id: 9002, source_change_id: 'C0284', captured_at: '2026-09-12T12:00:00Z', markets: { home_win: .18, draw: .23, away_win: .59 }, result_decision: 'AWAY', direction_strength: 'STRONG', primary_environment: 'NORMAL_SCORING', scoring_environment_probabilities: { low: .25, normal: .5, high: .25 }, scoring_environment_state: 'PRIMARY', expected_total_goals: 2.6, representative_score: '1-2', representative_score_probability: .10, raw_modal_score: '1-1', raw_modal_probability: .11, chronology_and_coverage_valid: true, decision_contract_version: 'c0284_fixture_decision_v01', decision_hash: 'fixture-2', top_scorelines: [{ score: '1-2', prob: .10 }] } },
  { match_id: 3, fpl_fixture_id: 3, kickoff_time: '2026-09-13T15:30:00Z', home_team: 'Man Utd', away_team: 'Man City', home_short: 'MUN', away_short: 'MCI', finished: false, home_score: null, away_score: null, prediction: { snapshot_id: 9003, source_change_id: 'C0284', captured_at: '2026-09-12T12:00:00Z', markets: { home_win: .34, draw: .30, away_win: .36 }, result_decision: 'NO_MEANINGFUL_EDGE', direction_strength: 'NO_MEANINGFUL_EDGE', primary_environment: 'NORMAL_SCORING', scoring_environment_probabilities: { low: .28, normal: .37, high: .35 }, scoring_environment_state: 'BLENDED_NEAR_TIE', expected_total_goals: 3.1, representative_score: '1-1', representative_score_probability: .115, raw_modal_score: '1-1', raw_modal_probability: .115, chronology_and_coverage_valid: true, decision_contract_version: 'c0284_fixture_decision_v01', decision_hash: 'fixture-3', top_scorelines: [{ score: '1-1', prob: .115 }] } },
];
...
function decisionEvidence(fixture: typeof currentFixtures[number]) { const item = (id: string, target: string, kind: 'MODEL_INPUT' | 'OBSERVED_CONTEXT', contribution: number) => ({ id, target, text: `${kind === 'MODEL_INPUT' ? 'Weighted strength' : 'Five-match form'} evidence for ${target}.`, kind, side: 'HOME', value: 1.2, weight: kind === 'MODEL_INPUT' ? .8 : 0, signed_contribution: contribution, sample: 5, cutoff: fixture.prediction.captured_at, source: kind === 'MODEL_INPUT' ? 'source_snapshot.home_team_state' : 'chronology-safe fixture reconstruction' }); return { contract_version: 'c0284_decision_evidence_v01', snapshot_id: fixture.prediction.snapshot_id, match_id: fixture.match_id, gameweek: 4, cutoff: fixture.prediction.captured_at, decision_hash: fixture.prediction.decision_hash, evidence_hash: `evidence-${fixture.match_id}`, conclusion: { result_decision: fixture.prediction.result_decision, result_margin: .18, home_projected_goals: 1.7, away_projected_goals: 1.1 }, targets: ['RESULT_DIRECTION', 'HOME_SCORING', 'AWAY_SCORING', 'SCORING_ENVIRONMENT', 'CONFIDENCE_UNCERTAINTY'].map((target, index) => ({ target, model_inputs: index === 1 ? [item('home-attack', target, 'MODEL_INPUT', .12), item('away-defence', target, 'MODEL_INPUT', -.08)] : index === 2 ? [item('away-attack', target, 'MODEL_INPUT', .05), item('home-defence', target, 'MODEL_INPUT', -.11)] : [], observed_context: index < 3 ? [item(`context-${index}`, target, 'OBSERVED_CONTEXT', 0)] : [], risks: index === 0 ? [{ id: 'direction-risk', target, text: 'Unbeaten opponent form tempers the selected direction.', severity: 'MEDIUM' }] : [] })), reliability: { classification: 'MODERATE', home: { result_sample: 5, xg_sample: 5, prior_weight: .2 }, away: { result_sample: 5, xg_sample: 4, prior_weight: .2 }, missing_is_not_zero: true }, synthesis: 'The weighted attack and defence inputs support the call, tempered by unbeaten opponent form.', player_implications: { scope: 'TEAM_SCORING_TO_PLAYER_DISCOVERY_ONLY', publication_authority: false, requires_p7_lineage: true, home: 'Home attacking upside requires the matching frozen player run.', away: 'Away attacking upside requires the matching frozen player run.' }, audit: { chronology_safe: true, decision_targets_complete: true, missing_is_not_zero: true } }; }
const fixtureFacts = { ok: true, gameweek: 4, facts_available: true, evidence_source: 'test', decision_evidence_contract: 'c0284_decision_evidence_v01', snapshot_run: { id: 1, as_of_gameweek: 4 }, fixtures: currentFixtures.map((fixture) => ({ match_id: fixture.match_id, gameweek: 4, kickoff_time: fixture.kickoff_time, alignment_basis: { snapshot_id: fixture.prediction.snapshot_id, captured_at: fixture.prediction.captured_at, source_change_id: fixture.prediction.source_change_id }, modal_facts: [], decision_evidence: decisionEvidence(fixture) })) };
...
test('no-edge fixture remains unforced and finished audit is understated', async ({ page }) => {
  await mockProductApis(page); await page.goto('/#matches');
  const derby = page.locator('.v3-match-card').filter({ hasText: 'Man Utd' }); await expect(derby.getByText('No clear edge', { exact: true })).toBeVisible(); await expect(derby.getByText('DRAW', { exact: true })).toHaveCount(0);
...
test('matchup modal keeps frozen probabilities and compact result comparison', async ({ page }) => {
  await mockProductApis(page); await page.goto('/?gw=4#matches'); const fixture = page.locator('.v3-match-card').first(); await expect(fixture).toBeVisible(); await fixture.getByRole('button', { name: 'Details' }).click(); const dialog = page.getByRole('dialog'); await expect(dialog).toBeVisible(); await expect(dialog.getByText('Probability board', { exact: true })).toBeVisible(); await expect(dialog.getByText('Decision evidence', { exact: true })).toBeVisible(); await expect(dialog.getByRole('heading', { name: /^Actual weighted model inputs \\d+$/ })).toBeVisible(); await expect(dialog.getByRole('heading', { name: /^Observed context · zero model effect \\d+$/ })).toBeVisible(); await expect(dialog.getByRole('heading', { name: /Targeted risks/ })).toBeVisible(); await expect(dialog.getByText(/Unbeaten opponent form tempers/)).toBeVisible(); await expect(dialog).toContainText('publication authority: no'); await dialog.getByRole('button', { name: 'Next matchup →' }).click(); await expect(dialog.getByRole('heading', { name: 'Sunderland vs Arsenal' })).toBeVisible();
});
