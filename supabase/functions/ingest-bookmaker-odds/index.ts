...
const getSelections=(market:any)=>{
  ...
  for(const o of Array.isArray(market?.odds)?market.odds:[]){
    ...
    if(o?.label&&Number(o?.odds)>1)out.push({key:slug(String(o.label)),name:String(o.label),line:Number.isFinite(line)?line:null,odds:Number(o.odds),kind:'label'});
    for(const[k,v]of Object.entries(o||{})){
      if(['hdp','label','odds','name'].includes(k))continue;
      const price=Number(v);
      if(Number.isFinite(price)&&price>1)out.push({key:slug(k),name:k,line:Number.isFinite(line)?line:null,odds:price,kind:'field'});
    }
  }
  return out;
};
const normalizeSnapshot=async(sb:any,r:any)=>{
  ...
  for(let i=0;i<rows.length;i+=500){
    const{data,error}=await sb.from('odds_market_selections').upsert(rows.slice(i,i+500),{onConflict:'raw_snapshot_id,market_key,selection_key,line',ignoreDuplicates:true}).select('id');
...
Deno.serve(async req=>{
  ...
  try{
    ...
    const{data:edgeRows,error:edgeError}=await sb.rpc('generate_correct_score_edge_observations',{p_gameweek:gw});
    const edgeGeneration=edgeError
      ?{status:'error',rows:0,error:edgeError.message}
      :{status:'success',rows:Number(edgeRows||0),error:null};
    const metadata={phase:'normalized_layer1_v3',from,to,normalized_selections:normalized,edge_layer:'correct_score_devig_v01',edge_generation:edgeGeneration,details};
    ...
    return new Response(JSON.stringify({ok:true,mode:'live',gameweek:gw,events_found:events.length,events_matched:matched,raw_payloads:rawCount,normalized_selections:normalized,edge_generation:edgeGeneration,details}),{headers:H});
