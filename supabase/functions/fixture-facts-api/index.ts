...
function topOutcome(markets:any):'H'|'D'|'A'|null{
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const [{data:matches,error:me},{data:teams,error:te},{data:card,error:ce},{data:modal,error:moe},{data:recent,error:rre},{data:preds,error:pe}]=await Promise.all([
      ...
      sb.from('current_production_fixture_prediction_v01').select('id,match_id,captured_at,markets,source_snapshot').eq('gameweek',gw)
    ]);
    if(me)throw me;if(te)throw te;if(ce)throw ce;if(moe)throw moe;if(rre)throw rre;if(pe)throw pe;
