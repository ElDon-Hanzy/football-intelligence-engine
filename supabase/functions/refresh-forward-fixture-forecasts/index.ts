...
function canonicalState(row:any){
  ...
  const rawCurrentAttack=row.current_xg_for_90==null?null:.75*Number(row.current_xg_for_90)+.25*Number(row.current_goals_for_90);
  const rawCurrentDefence=row.current_xg_against_90==null?null:.80*Number(row.current_xg_against_90)+.20*Number(row.current_goals_against_90);
  // Sampling reliability is not previous-season weight. It shrinks noisy current-season
  // rates toward this season's league mean using two neutral pseudo-matches.
  const reliabilityPseudoMatches=2,leagueMean=1.35;
  const regularize=(value:number|null)=>value==null?null:(matches*value+reliabilityPseudoMatches*leagueMean)/(matches+reliabilityPseudoMatches);
  const currentAttack=regularize(rawCurrentAttack),currentDefence=regularize(rawCurrentDefence);
  ...
  return {...row,matches,weights,rawCurrentAttack,rawCurrentDefence,currentAttack,currentDefence,reliabilityPseudoMatches,attack:blend(currentAttack,priorAttack),defence:blend(currentDefence,priorDefence)};
}
...
Deno.serve(async(req)=>{try{
  ...
  for(const f of fixtures||[]){
    ...
    const duplicate=(byMatch.get(Number(f.id))||[]).some((e:any)=>e.source_snapshot?.generator==='forward_fixture_v0.3.0_c0284_current_season'&&Number(e.source_snapshot?.home_team_state?.id)===Number(hs.id)&&Number(e.source_snapshot?.away_team_state?.id)===Number(as.id)&&Number(e.home_lambda)===home&&Number(e.away_lambda)===away);
    if(duplicate){skipped.push({match_id:f.id,reason:'unchanged'});continue}
    const b=bundle(home,away); const forecastHash=await sha256(`forward_fixture_v0.3.0_c0284_current_season|${f.id}|${hs.id}|${as.id}|${home}|${away}`);
    const statePayload=(s:any)=>({id:s.id,as_of:s.as_of,completed_matches:s.matches,current_season_weight:s.weights.current,prior_season_weight:s.weights.prior,weight_band:s.weights.band,current_goals_for_90:s.current_goals_for_90,current_goals_against_90:s.current_goals_against_90,current_xg_for_90:s.current_xg_for_90,current_xg_against_90:s.current_xg_against_90,raw_current_attack:s.rawCurrentAttack,raw_current_defence:s.rawCurrentDefence,current_reliability_pseudo_matches:s.reliabilityPseudoMatches,reliability_target:'CURRENT_SEASON_LEAGUE_MEAN',regularized_current_attack:s.currentAttack,regularized_current_defence:s.currentDefence,canonical_attack:s.attack,canonical_defence:s.defence,prior_source:s.prior_source,evidence_hash:s.evidence_hash});
    rows.push({match_id:f.id,gameweek:gw,model_version_id:mv.id,captured_at:observedAt,kickoff_time:f.kickoff_time,is_pre_kickoff:true,frozen:false,home_lambda:home,away_lambda:away,score_matrix:b.score_matrix,top_scorelines:b.top_scorelines,markets:b.markets,confidence:null,change_reasons:[{type:'canonical_current_season_state',generator:'forward_fixture_v0.3.0_c0284_current_season',note:'Current-season xG and goals are the performance baseline; current-season sampling noise is regularized toward the current league mean; prior season is capped at 25% and decays to zero at nine matches.'}],source_snapshot:{change_id:'C0284',engine:'shared_scoreline_v0.3.0_c0284_current_season',generator:'forward_fixture_v0.3.0_c0284_current_season',forecast_hash:forecastHash,model_version:'0.1.3',captured_at:observedAt,home_team_state:statePayload(hs),away_team_state:statePayload(as),formula:{league_xg:1.35,goal_scale:1.05,attack_power:.90,defence_power:.70,home_multiplier:1.04,away_multiplier:.97,current_attack_mix:{xg:.75,goals:.25},current_defence_mix:{xga:.80,goals_against:.20},current_season_reliability_pseudo_matches:2,reliability_target:'CURRENT_SEASON_LEAGUE_MEAN',prior_cap_after_first_match:.25,prior_zero_after_matches:9},legacy_cross_season_l5_consumed:false,legacy_cross_season_l10_consumed:false,legacy_l20_consumed:false,missing_is_not_zero:true}});
  }
  ...
  return new Response(JSON.stringify({ok:true,gameweek:gw,fixtures:fixtures.length,candidates:rows.length,inserted,skipped,observed_at:observedAt,generator:'forward_fixture_v0.3.0_c0284_current_season',model_version_id:mv.id,pre_kickoff_only:true,historical_forecasts_rewritten:false}),{headers:H});
