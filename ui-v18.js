(function(){
  ...
  function phrase(x,lead){const f=family(x);if(f==='MODEL_THESIS')return 'the projected scoring gap';if(f==='VENUE_FORM')return 'the home/away record';if(f==='STREAK_PROFILE')return 'the recent streak profile';if(f==='MATCHUP_XG')return 'the recent chance-quality matchup';if(f==='CURRENT_SEASON')return 'this season’s chance data';if(f==='RESULT_PROCESS')return 'the gap between results and underlying xG';if(f==='SCORE_DEFENCE')return 'the scoring-versus-clean-sheet pattern';return `${lead}'s supporting evidence`}
