...
Deno.serve(async(req)=>{if(req.method==='OPTIONS')return new Response('ok',{headers:cors});try{
  ...
  const [{data:allPreds},{data:allPlayers},{data:teams},{data:states},prevStatesRes,{data:dbMatches},{data:fixturePreds}]=await Promise.all([
    ...
    sb.from('fixture_prediction_snapshots').select('match_id,captured_at,kickoff_time,is_pre_kickoff,home_lambda,away_lambda,top_scorelines,markets,confidence').eq('gameweek',gw).eq('is_pre_kickoff',true).order('captured_at',{ascending:false})
  ]);
  ...
  const latestFixturePred=new Map<number,any>();for(const p of fixturePreds||[]){if(!latestFixturePred.has(Number(p.match_id)))latestFixturePred.set(Number(p.match_id),p)}
