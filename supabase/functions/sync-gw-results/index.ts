...
Deno.serve(async(req)=>{try{
  ...
  if(!gw){
    const br=await fetch('https://fantasy.premierleague.com/api/bootstrap-static/',{headers:{'User-Agent':'FootballIntelligence/0.3'}});
  ...
  const [lr,fr]=await Promise.all([
    fetch(`https://fantasy.premierleague.com/api/event/${gw}/live/`,{headers:{'User-Agent':'FootballIntelligence/0.3'}}),
    fetch(`https://fantasy.premierleague.com/api/fixtures/?event=${gw}`,{headers:{'User-Agent':'FootballIntelligence/0.3'}})
  ]);
  ...
  for(const f of fixtures||[]){
    ...
    if(!matchId)continue;
    const {error}=await sb.from('matches').update({home_score:i(f.team_h_score),away_score:i(f.team_a_score),finished:Boolean(f.finished===true||f.finished_provisional===true),raw:f,updated_at:updatedAt}).eq('id',matchId);
    if(error)throw error;
  }
  const {data:teamEvidenceRows,error:tie}=await sb.rpc('refresh_current_season_team_intelligence');
  if(tie)throw new Error(`team intelligence refresh: ${tie.message}`);
  ...
  if(last?.metadata?.payload_hash===payloadHash&&Array.isArray(last?.metadata?.finished_fixture_ids)){
    return Response.json({ok:true,gameweek:gw,unchanged:true,result_run_id:last.id,finished_fixture_ids:finishedFixtureIds,matches_reconciled:(dbMatches||[]).length,current_season_team_rows_inserted:Number(teamEvidenceRows||0)});
  }
  ...
  try{await fetch(`${base}/functions/v1/audit-gw?gw=${gw}`,{headers:{'User-Agent':'FootballIntelligence/0.3'}})}catch(_){/* audit can retry later */}
  return Response.json({ok:true,gameweek:gw,result_run_id:run.id,is_final:isFinal,players:rows.length,fixture_count:fixtures.length,finished_fixture_ids:finishedFixtureIds,matches_reconciled:(dbMatches||[]).length,current_season_team_rows_inserted:Number(teamEvidenceRows||0)});
