...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const [
      ...
      sb.from('fpl_manager_plans').select('id,gameweek,captured_at,status,horizon,transfers,captain_player_id,vice_player_id,starting_xi,bench_order,chip,gw_expected_xi_points,expected_gain_current_gw,expected_gain_horizon,risk_level,rationale,source,supersedes_id').order('gameweek', { ascending: true }).order('captured_at', { ascending: false }).order('id', { ascending: false }),
      sb.from('fpl_manager_state_snapshots').select('id,gameweek,captured_at,free_transfers,bank_tenths,acquisition_squad_cost_tenths,source,evidence').order('gameweek', { ascending: true }).order('captured_at', { ascending: false }).order('id', { ascending: false }),
      sb.from('fpl_actual_manager_decisions').select('id,gameweek,captured_at,captain_player_id,vice_player_id,starting_xi,bench_order,chip,source,notes,correction_of_id').order('gameweek', { ascending: true }).order('captured_at', { ascending: false }).order('id', { ascending: false }),
      sb.from('current_fpl_live_plan_v01').select('id,gameweek,horizon,captured_at,publication_stage,publication_status,final_status,execution_authorized,prediction_run_id,manager_state_id,optimizer_run_id,autonomous_gate_run_id,plan,alternatives,research_inputs,blockers,freshness,layer_lineage,source,historical_forecasts_rewritten').order('gameweek', { ascending: true }),
    ]);
    ...
    const availableGameweeks = [...new Set([...(plans || []), ...(states || []), ...(actualDecisions || []), ...(livePublications || [])].map((row: any) => Number(row.gameweek)).filter((gw: number) => Number.isInteger(gw) && gw >= 1 && gw <= 38))].sort((a, b) => a - b);
    const gameweek = requested >= 1 && requested <= 38 ? requested : availableGameweeks.length ? availableGameweeks[availableGameweeks.length - 1] : null;
    const latestForGameweek = (rows: any[], gw: number | null) => gw == null ? null : rows.filter((row: any) => Number(row.gameweek) === gw).sort((a: any, b: any) => new Date(b.captured_at).getTime() - new Date(a.captured_at).getTime() || Number(b.id) - Number(a.id))[0] || null;
    ...
    return new Response(JSON.stringify({
      ...
      semantics: {
        primary_plan_is_live_publication_when_available: true,
